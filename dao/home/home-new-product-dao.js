...
module.exports = {
    ...
    insert(recommendProduct) {
        ...
        let sql = `insert into sms_home_new_product(product_id,product_name,recommend_status,sort)values(?,?,?,?)`;
    ...
    update(recommendSubject) {
        ...
        let sql = `update sms_home_new_product set ${set} where id=${id}`;
        return Mysql.transExcute(sql);
    },
    //删
    delete(id) {
        let sql = `delete from sms_home_new_product where id=?`;
    ...
    list(productName, recommendStatus, pageNum, pageSize) {
        ...
        let count = `select count(*) as count from sms_home_new_product ${where}`;
        let sql = `select ${field} from sms_home_new_product ${where} ${order} ${limit};`;
